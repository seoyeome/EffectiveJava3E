...
- 호출될 때마다 인스턴스를 새로 생성하지 않아도 됨
    - 이로 인해 인스턴스 미리 만들어놓아 불필요한 객체 생성을 피할 수 있음
      - ```
        ...
        // 이때 기본 생성자 타입으로 하는 경우는
        Cat usaCat = new USACat(); -> USACat객체가 미리 정의되있어야 함
        // but, 정적 팩토리 메소드 방식으로 하는 경우는 of메소드 내의 switch를 통해
        Cat usaCat = new Cat("USA"); -> 이렇게 전달 인자만 바꿔주면 쉽게 확장 가능하고 + 불필요한 객체 생성을 피할 수 있는 것 
...
- try-with-resources 에서도 catch 절을 쓸 수 있음
